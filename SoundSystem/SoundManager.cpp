...
bool SoundManager::IsPlaying(const SoundID& l_id) const {
	...
	const auto& sound = container->second.find(l_id);
...
SoundProps* SoundManager::GetSoundProperties(const std::string& l_soundName) {
	...
	if (properties == m_properties.end()) {
		...
		m_properties.insert({ l_soundName, m_properties.find(l_soundName)->second });
	}
	return &m_properties.find(l_soundName)->second;
}
